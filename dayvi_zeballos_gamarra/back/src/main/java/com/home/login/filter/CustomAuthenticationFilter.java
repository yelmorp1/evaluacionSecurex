...
public class CustomAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
	...
	protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain,
		...
		User user = (User) authentication.getPrincipal(); // se castea porque retorna un objeto
		...
		log.info((new Date(System.currentTimeMillis() + 1 * 60 * 1000).toString()));
		String refresh_token = JWT.create().withSubject(user.getUsername())
				.withExpiresAt(new Date(System.currentTimeMillis() + 10 * 60 * 1000))
				.withIssuer(request.getRequestURL().toString())
				// .withClaim("roles",
				// user.getAuthorities().stream().map(GrantedAuthority::getAuthority).collect(Collectors.toList())) No es necesario el rol
				.sign(algorithm); 
